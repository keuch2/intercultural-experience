...
class ProgramAssignmentController extends Controller
{
    ...
    /**
     * Bulk assign programs to multiple users.
     */
        ...
        try {
            ...
            foreach ($userIds as $userId) {
                ...
                if ($existingAssignment) {
                    ...
                    $errors[] = "Usuario {$user->name} ya tiene una asignación para este programa.";
                    continue;
                }

                // Verificar capacidad
                if (!$program->canAcceptMoreParticipants()) {
                    $errors[] = "El programa alcanzó su capacidad máxima.";
                    break;
                }
            ...
            $message = "Se asignaron {$successCount} usuarios exitosamente.";
            if (!empty($errors)) {
            ...
            DB::rollback();
            return back()->withErrors([
                'error' => 'Error en la asignación masiva: ' . $e->getMessage()
            ]);
        }
    }
