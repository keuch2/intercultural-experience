...
<div class="card shadow-sm mb-4">
    ...
    <div class="card-body">
        ...
        <div class="table-responsive"><table class="table table-sm align-middle mb-0">
            ...
            <tbody>@foreach($offers as $o)<tr><td>{{ $o->employer_name }}</td><td>{{ $o->city }}, {{ $o->state }}</td><td class="text-center"><span class="badge bg-success">{{ $o->positions_available }}</span></td>
                <td class="text-end"><form method="POST" action="{{ route('admin.program.job-pool.assign', [$program->slug, $o->id]) }}" onsubmit="return confirm('¿Asignar {{ $o->employer_name }} a este participante?')">@csrf<input type="hidden" name="process_id" value="{{ $process->id }}"><button class="btn btn-sm btn-outline-primary py-0"><i class="fas fa-user-plus me-1"></i> Asignar</button></form></td></tr>@endforeach</tbody>
        </table></div>
...
<div class="card shadow-sm mb-4">
    ...
    <div class="card-body">
        ...
        <div class="border-bottom py-2 small d-flex justify-content-between"><div><strong>{{ $ev->label }}</strong> @if($ev->offer)— {{ $ev->offer->employer_name }} ({{ $ev->offer->city }})@endif @if(!empty($ev->payload['reason']))<span class="text-muted">· {{ $ev->payload['reason'] }}</span>@endif</div><span class="text-muted text-nowrap">{{ $ev->created_at->format('d/m/Y H:i') }} · {{ $ev->actor?->name ?? ucfirst($ev->actor_type) }}</span></div>
        @endforeach
    </div>
</div>
@endif
