...
<div class="card shadow-sm mb-4">
    ...
    <div class="card-body">
        ...
        <div class="table-responsive"><table class="table table-sm align-middle mb-0">
            ...
            <tbody>@foreach($offers as $o)<tr><td><strong>{{ $o->display_name }}</strong>@if($o->job_title)<div class="small text-muted">{{ $o->employer_name }}</div>@endif @if($o->application_deadline)<div class="small text-muted">hasta {{ $o->application_deadline->format('d/m/Y') }}</div>@endif</td><td>{{ $o->city }}, {{ $o->state }}</td><td class="text-center"><span class="badge bg-success">{{ $o->positions_available }}</span></td>
