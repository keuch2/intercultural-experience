@php
    $logs = $tabData['logs'];
    $types = $tabData['types'];
    $labels = ['arrival_followup' => 'Seguimiento de llegada', 'monthly_followup' => 'Seguimiento mensual', 'program_followup' => 'Seguimiento durante el programa', 'incident' => 'Incidente', 'employer_change' => 'Cambio de empleador', 'experience_evaluation' => 'Evaluación de experiencia', 'final_evaluation' => 'Evaluación final'];
    ...
    <div class="card-body">
        ...
        <div class="border rounded p-3 mb-2 {{ $log->severity === 'critical' ? 'border-danger' : ($log->severity === 'high' ? 'border-warning' : '') }}">
            <div class="d-flex justify-content-between align-items-start">
                <div><strong>{{ $log->title }}</strong>@if($log->follow_up_number)<span class="badge bg-primary ms-1">#{{ $log->follow_up_number }}</span>@endif @if($log->severity)<span class="badge bg-{{ $sevColor[$log->severity] ?? 'secondary' }} ms-1">{{ ucfirst($log->severity) }}</span>@endif<small class="text-muted ms-2">{{ $log->log_date->format('d/m/Y') }}</small>@if($log->loggedBy)<small class="text-muted"> — {{ $log->loggedBy->name }}</small>@endif</div>
...
<div class="modal fade" id="addSupportLogModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form method="POST" action="{{ route('admin.program.support.store', [$program->slug, $process->id]) }}">@csrf
        ...
        <div class="modal-body"><div class="row g-3">
            <div class="col-md-6"><label class="form-label small">Tipo *</label><select name="log_type" class="form-select form-select-sm" required id="supportLogType">@foreach($types as $type)<option value="{{ $type }}">{{ $labels[$type] ?? Str::headline($type) }}</option>@endforeach</select></div>
